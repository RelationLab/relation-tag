...
vacuum static_total_data${tableSuffix};
...
set
    avg_birthday = (select
                     ...
                     from address_labels_json_gin${tableSuffix} aljg
                     left join (select
                                    address,
                                ...
                                from
                                    (
                                        ...
                                        from
                                            contract c) aiout
                                group by
                                    address) ai on (ai.address=aljg.address)
...
-----更新action_range
update static_total_data${tableSuffix} set action_range= (
    select
        ('{' || string_agg(json_text::text, ',')|| '}')::jsonb
    from
        (
