...
select address,token,balance_usd,volume_usd from  (select distinct th.address, token, th.balance * price as balance_usd, total_transfer_all_volume* price as volume_usd
   from token_holding th  inner join white_list_erc20_temp wle  on th.token = wle.address and ignored = false where  th.token in (select token_id from dim_rank_token)) budout
