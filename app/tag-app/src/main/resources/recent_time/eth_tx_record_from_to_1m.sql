...
from
    (
        ...
        group by
            to_address) atb
        left join (select * from  recent_time where recent_time.recent_time_code='1m' ) recent_time on
        (atb.block_height >= recent_time.block_height)
where address !=''
group by
    address,
    recent_time.recent_time_code;
insert into tag_result(table_name,batch_date)  SELECT 'eth_tx_record_from_to_1m' as table_name,to_char(current_date ,'YYYY-MM-DD')  as batch_date;
