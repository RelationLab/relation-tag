...
vacuum
address_label_token_time_first_stake;

insert into public.address_label_token_time_first_stake(address, label_type, label_name, data, wired_type, updated_at,
                                                        "group", level, category, trade_type, project, asset, bus_type)
select s1.address,
       s1.label_type,
       s1.label_type || '3i' as label_name,
       rn                    as data,
       'DEFI'                as wired_type,
       now()                 as updated_at,
       't'                   as "group",
            ...
            where type = 'stakelp'
              ...
              and address not in (select address from exclude_address)) a1
               inner join dim_project_token_type_temp a2
                          on
                                      a1.token = a2.token
                                  and a1.project = a2.project   and a2.wired_type = 'DEFI'
                                  and a2.data_subject = 'FIRST_MOVER_STAKING') s1
where s1.rn <= 100;
insert into tag_result(table_name, batch_date)
SELECT 'address_label_token_time_first_stake' as table_name, '${batchDate}' as batch_date;
