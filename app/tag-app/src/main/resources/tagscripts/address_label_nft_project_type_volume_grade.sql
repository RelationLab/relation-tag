...
vacuum
address_label_nft_project_type_volume_grade;

insert into public.address_label_nft_project_type_volume_grade(address, label_type, label_name, data, wired_type,
...
select address,
       ...
       label_type || case
                         when volume_usd >= 100
                             and volume_usd < 1000 then '5a'
                         when volume_usd >= 1000
                             and volume_usd < 10000 then '5b'
                         when volume_usd >= 10000
                             and volume_usd < 50000 then '5c'
                         when volume_usd >= 50000
                             and volume_usd < 100000 then '5d'
                         when volume_usd >= 100000
                             and volume_usd < 500000 then '5e'
                         when volume_usd >= 500000 then '5f'
           end                                     as label_name,
       volume_usd                                  as data,
       'NFT'                                       as wired_type,
       now()                                       as updated_at,
