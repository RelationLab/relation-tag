...
vacuum
address_label_web3_type_count_grade;
...
select address,
       ...
       label_type || case
                         when total_transfer_count >= 1
                             and total_transfer_count < 10 then '9a'
                         when total_transfer_count >= 10
                             and total_transfer_count < 40 then '9b'
                         when total_transfer_count >= 40
                             and total_transfer_count < 80 then '9c'
                         when total_transfer_count >= 80
                             and total_transfer_count < 120 then '9d'
                         when total_transfer_count >= 120
                             and total_transfer_count < 160 then '9e'
                         when total_transfer_count >= 160
                             and total_transfer_count < 200 then '9f'
                         when total_transfer_count >= 200
                             and total_transfer_count < 400 then '9g'
                         when total_transfer_count >= 400
                             and total_transfer_count < 619 then '9h'
                         when total_transfer_count >= 619 then '9i'
           end                                              as label_name,
...
from (
         ...
         select a1.address,
                ...
                recent_time_code
         from web3_transaction_record_summary_temp a1
...
insert
into public.address_label_crowd_web3_active_users(address,
                                                  label_type,
                                                  label_name,
                                                  data,
                                                  wired_type,
...
SELECT 'address_label_web3_type_count_grade' as table_name, '${batchDate}' as batch_date;