...
insert into public.address_label_web3_type_count_grade(address,label_type,label_name,data,wired_type,updated_at,"group",level,category,trade_type,project,asset,bus_type, recent_time_code )
select
    address,
    label_type,
    label_type || '_' || case
...
where
  ...
  and address not in (
    ...
    from
        exclude_address);
insert into tag_result(table_name,batch_date)  SELECT 'address_label_web3_type_count_grade' as table_name,'${batchDate}'  as batch_date;