...
insert into public.address_label_token_balance_top (address,label_type,label_name,updated_at)
    select
    s1.address,
    s1.label_type,
	...
	order by
        ...
        from (select  address,
                      token,
                      sum(balance_usd) as balance_usd from
            (
                select
                    address,
                    token,
            ...
            ) totala  group by  address,token) a1
