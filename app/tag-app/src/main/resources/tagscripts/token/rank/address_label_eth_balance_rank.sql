...
insert into public.address_label_eth_balance_rank (address,label_type,label_name,updated_at)
    select tb1.address ,
       tb2.label_type,
            ...
            from (select a1.id
                  ...
                  from (select md5(cast(random() as varchar)) as id
                       ...
                       (select count(distinct address) as count_sum_total
                        from token_balance_volume_usd  where token = 'eth'   and balance_usd >= 100) as a10
                       on 1 = 1) as a2) as t1
     ) tb1
         inner join
