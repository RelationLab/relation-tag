            ...
            from (select a1.id
                  ...
                  from (select md5(cast(random() as varchar)) as id
                       ...
                       (select count(distinct address) as count_sum_total
                        from token_balance_volume_usd where token = '0xdac17f958d2ee523a2206206994597c13d831ec7' and volume_usd >= 100) as a10
                       on 1 = 1) as a2) as t1) tb1
         inner join
     dim_rule_content tb2
     on
             tb1.token = tb2.token
where  tb2.data_subject = 'volume_rank' and tb2.token_type='token' and  zb_rate <= 0.1;