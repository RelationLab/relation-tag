...
vacuum address_label_univ3_balance_top;

insert into public.address_label_univ3_balance_top(address,label_type,label_name,data,wired_type,updated_at,"group",level,category,trade_type,project,asset,bus_type)
select
    s1.address,
    s1.label_type,
                ...
                from
                    (
                        ...
                        where
                          ...
                          and balance_usd >= 100
                          and type = 'lp' and address not in (select address from exclude_address)
                    ) totala
                group by
                    address,
                    token) a1
...
where
        s1.rn <= 100;
insert into tag_result(table_name,batch_date)  SELECT 'address_label_univ3_balance_top' as table_name,to_char(current_date ,'YYYY-MM-DD')  as batch_date;
