...
vacuum
address_label_token_count_grade;
...
from (select address,
             ...
             recent_time_code
      from token_holding_vol_count_temp th1
      ...
      from (select address,
                   total_transfer_count,
                   recent_time_code
            from eth_holding_vol_count_temp th
            where total_transfer_count >= 1
            union all
            select address,
                   total_transfer_count,
                   recent_time_code
            from token_holding_vol_count_temp th
            where total_transfer_count >= 1
              and th.token in (select token_id from dim_rank_token_temp)
            union all
            select address,
            ...
            where th.project = '0xc36442b4a4522e871399cd717abdd847ab11fe88'
              ...
              and th.token in (select token_id from dim_rank_token_temp)) th2
      group by address,
               recent_time_code) a1
         inner join
