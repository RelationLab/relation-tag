...
insert into public.address_label_nft_transfer_count_grade(address,label_type,label_name,updated_at)
    select address,
       label_type,
       label_type||'_'||case
           when sum_count >= 1
    ...
    from (
		...
		and (a2.project ='' or a2.project ='ALL') where  total_transfer_count >= 1
         group by a1.address,
             a2.seq_flag,
             a2.label_type
         union all
         ...
         group by a1.address,
             a2.seq_flag,
             a2.label_type) t where sum_count >= 1;