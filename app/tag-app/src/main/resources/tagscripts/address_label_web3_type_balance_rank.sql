...
vacuum
address_label_web3_type_balance_rank;

insert into public.address_label_web3_type_balance_rank(address, label_type, label_name, data, wired_type, updated_at,
                                                        "group", level, category, trade_type, project, asset, bus_type)
select address,
       label_type,
       label_type || case
                         when zb_rate > 0.01
                             and zb_rate <= 0.025 then '8h'
                         when zb_rate > 0.001
                             and zb_rate <= 0.01 then '8i'
                         when zb_rate > 0.025
                             and zb_rate <= 0.1 then '8g'
                         when zb_rate <= 0.001 then '8j'
           end                                        as label_name,
       zb_rate                                        as data,
       'WEB3'                                         as wired_type,
       now()                                          as updated_at,
                  ...
                  from (select a1.address,
					...
					order by
						...
						address asc) as count_sum
                             ...
                             on
                                         a10.type = a1.type
                                     and a10.project = a1.project) as a2) as t1) tb1
               inner join
           dim_project_type_temp tb2
           on
                       tb1.project = tb2.project
                   and tb1.type = tb2.type
      where tb2.data_subject = 'balance_rank'
        and zb_rate <= 0.1) t;
insert into tag_result(table_name, batch_date)
SELECT 'address_label_web3_type_balance_rank' as table_name, '${batchDate}' as batch_date;
