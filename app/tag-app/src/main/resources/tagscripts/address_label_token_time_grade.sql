...
vacuum address_label_token_time_grade;

insert into public.address_label_token_time_grade(address,label_type,label_name,data,wired_type,updated_at,"group",level,category,trade_type,project,asset,bus_type)
    select
    a1.address,
    a2.label_type,
    ...
    (
        select
            ...
            counter
        from
            snapshot_token_holding_time tbvutk) a1
        inner join
    dim_rule_content a2
    on
            a1.token = a2.token
...
insert into tag_result(table_name,batch_date)  SELECT 'address_label_token_time_grade' as table_name,to_char(current_date ,'YYYY-MM-DD')  as batch_date;
