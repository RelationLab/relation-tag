...
vacuum
address_label_univ3_balance_provider;

insert into public.address_label_univ3_balance_provider(address, label_type, label_name, data, wired_type, updated_at,
                                                        "group", level, category, trade_type, project, asset, bus_type)
select s1.address,
       s1.label_type,
       s1.label_type || '0k' as label_name,
       rn                    as data,
       'DEFI'                as wired_type,
       now()                 as updated_at,
       'b'                   as "group",
...
SELECT 'address_label_univ3_balance_provider' as table_name, '${batchDate}' as batch_date;
