...
vacuum
address_label_nft_transfer_volume_grade;

insert into public.address_label_nft_transfer_volume_grade(address, label_type, label_name, data, wired_type,
...
select address,
       ...
       label_type || case
                         when volume >= 1
                             and volume <= 2 then '5a'
                         when volume >= 3
                             and volume <= 6 then '5b'
                         when volume >= 7
                             and volume <= 20 then '5c'
                         when volume >= 21
                             and volume <= 100 then '5d'
                         when volume >= 101
                             and volume <= 200 then '5e'
                         when volume >= 201 then '5f'
           end                              as label_name,
       volume                               as data,
       'NFT'                                as wired_type,
       now()                                as updated_at,
...
SELECT 'address_label_nft_transfer_volume_grade' as table_name, '${batchDate}' as batch_date;