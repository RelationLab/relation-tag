...
vacuum address_label_nft_time_top;

insert into public.address_label_nft_time_top(address,label_type,label_name,data,wired_type,updated_at,"group",level,category,trade_type,project,asset,bus_type)
select
    s1.address,
    s1.label_type,
        ...
        from
            (
                select
                    ...
                    first_tx_time
                from
                    nft_holding_time_temp where  address not in (select address from exclude_address)
...
where
        s1.rn <= 100 ;
insert into tag_result(table_name,batch_date)  SELECT 'address_label_nft_time_top' as table_name,'${batchDate}'  as batch_date;
