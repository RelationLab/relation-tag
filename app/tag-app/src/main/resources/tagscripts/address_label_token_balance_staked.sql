...
insert into public.address_label_token_balance_staked(address,label_type,label_name,data,wired_type,updated_at,"group",level,category,trade_type,project,asset,bus_type)
        ...
        from
            (
                select
                    *
                from
                    dex_tx_volume_count_summary_stake_temp
                where
                        type = 'stakelp'
    ...
    where
        s1.rn <= 200;
insert into tag_result(table_name,batch_date)  SELECT 'address_label_token_balance_staked' as table_name,'${batchDate}'  as batch_date;
