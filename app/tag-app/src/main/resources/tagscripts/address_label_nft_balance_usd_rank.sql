...
vacuum address_label_nft_balance_usd_rank;

insert into public.address_label_nft_balance_usd_rank(address,label_type,label_name,data,wired_type,updated_at,"group",level,category,trade_type,project,asset,bus_type)
select
    address,
    label_type,
    label_type || '_' || case
...
insert into public.address_label_crowd_nft_whale(address,label_type,label_name,data,wired_type,updated_at,"group",level,category,trade_type,project,asset,bus_type)
 select
     distinct a1.address ,
        ...
        'CROWD' as bus_type
       from (
                select address from address_label_nft_balance_usd_rank
...
insert into tag_result(table_name,batch_date)  SELECT 'address_label_nft_balance_usd_rank' as table_name,'${batchDate}'  as batch_date;
