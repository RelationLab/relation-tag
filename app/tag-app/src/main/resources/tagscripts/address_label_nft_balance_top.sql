...
vacuum address_label_nft_balance_top;
...
select
		...
		from
			...
			from
				...
				select
					...
					balance
				from
                    snapshot_nft_holding where  address not in (select address from exclude_address)
    ...
    where
    s1.rn <= 100 ) t;
insert into tag_result(table_name,batch_date)  SELECT 'address_label_nft_balance_top' as table_name,to_char(current_date ,'YYYY-MM-DD')  as batch_date;
