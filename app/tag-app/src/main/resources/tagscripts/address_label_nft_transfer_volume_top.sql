...
vacuum
address_label_nft_transfer_volume_top;

insert
into public.address_label_nft_transfer_volume_top(address,
                                                  label_type,
                                                  label_name,
                                                  data,
                                                  wired_type,
...
from (select address,
      ...
      from (select a1.address,
		...
		order by
			...
			address asc) as rn, recent_time_code
            from (select address,
                         seq_flag,
                  ...
                  from (select address,
                        ...
                        where total_transfer_volume >= 1
                          ...
                          and token in (select token_id
                                        from dim_project_token_type_rank_temp dpttr)
                        union all
                        ...
                        where total_transfer_volume >= 1
                          and address not in (select address
                                              from exclude_address)
                          and token in (select token_id
                                        from dim_project_token_type_rank_temp dpttr)) totala
...
SELECT 'address_label_nft_transfer_volume_top' as table_name, '${batchDate}' as batch_date;