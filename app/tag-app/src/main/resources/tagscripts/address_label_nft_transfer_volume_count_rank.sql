...
vacuum
address_label_nft_transfer_volume_count_rank;

insert into public.address_label_nft_transfer_volume_count_rank(address, label_type, label_name, data, wired_type,
      ...
      from (select t1.address,
            ...
            from (select a2.address,
                  ...
                  from (select a1.address,
					    ...
					    order by total_transfer_volume desc,address asc) as count_sum, row_number() over(partition by seq_flag,a1.recent_time_code
					    order by total_transfer_count desc,address asc) as transfer_count_sum, a1.recent_time_code
          ...
          and dptt.wired_type='NFT' and tb1.recent_time_code = dptt.recent_code)
      where tb1.total_transfer_volume >= 1
        and zb_rate <= 0.001
        and zb_rate_transfer_count <= 0.001) t;
insert into tag_result(table_name, batch_date)
SELECT 'address_label_nft_transfer_volume_count_rank' as table_name, '${batchDate}' as batch_date;
